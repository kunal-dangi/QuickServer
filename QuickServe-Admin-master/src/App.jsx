...
function App() {
  ...
  const todaysOrders = orders.filter(o => new Date(o.date).toISOString().split('T')[0] === todayDateString);
  ...
  const lastOrderTime = orders.length > 0 
    ? new Date(orders[0].date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) 
    : '--:--';
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center border-b border-dashed border-gray-200 pb-5 mb-5 gap-4 sm:gap-0">
                  ...
                  <div className="text-left sm:text-right ml-12 sm:ml-0">
                    ...
                    <div className="text-gray-700 text-sm mt-0.5 font-medium">{formatDate(order.date)}</div>
                  </div>
                </div>
